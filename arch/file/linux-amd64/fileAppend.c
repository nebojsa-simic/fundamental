...
static AsyncStatus poll_mmap_append(AsyncResult *result)
{
	...
	if (!state->file_extended) {
		...
		if (syscall2(SYS_ftruncate, state->file_descriptor, (long)new_size) < 0) {
	...
	if (!state->mapped) {
		...
		if ((long)mapped < 0) {
			result->error = fun_error_result(1, "Failed to mmap file for append");
			final_status = ASYNC_ERROR;
			goto cleanup;
		}
...
cleanup:
	if (state->mapped_address && state->mapped_address != (void *)-1)
		syscall2(SYS_munmap, (long)state->mapped_address, (long)state->view_size);
...
static AsyncStatus poll_ring_append(AsyncResult *result)
{
	...
	if (!state->ring_initialized) {
		...
		void *sq_mmap = (void *)syscall6(SYS_mmap, 0, (long)sq_size, 0x3, 0x11,
										 ring_fd, 0);
		if ((long)sq_mmap < 0) {
	...
	if (cqe->user_data == 1) {
		if (cqe->res < 0) {
			result->error = fun_error_result(-cqe->res, "io_uring append failed");
			final_status = ASYNC_ERROR;
		} else {
