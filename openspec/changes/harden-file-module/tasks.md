...
- [x] 1.1 Create `src/file/overflow_check.h` with inline overflow check helpers
- [x] 1.2 Implement `check_overflow_add(uint64_t a, uint64_t b, uint64_t *result)`
- [x] 1.3 Implement `check_overflow_sub(uint64_t a, uint64_t b, uint64_t *result)`
- [x] 1.4 Implement `check_overflow_mul(uint64_t a, uint64_t b, uint64_t *result)`
...
- [x] 2.3 Add `cqe_consumed` flag to ring-based state structs
...
- [x] 4.1 Define `FILE_LOCK_TIMEOUT_MS` default constant (5000)
- [x] 4.2 Define `FILE_LOCK_RETRY_INTERVAL_MS` constant (100)
- [x] 4.3 Create `fun_file_lock_with_timeout(String filePath, uint32_t timeout_ms)`
- [x] 4.4 Implement non-blocking lock with `LOCK_EX | LOCK_NB`
- [x] 4.5 Implement retry loop with timeout check
...
- [x] 6.1 Update `fileReadMmap.c` to check `bytes_to_read <= output.capacity`
...
- [x] 10.3 Run `./code-format.sh` on all modified files
- [x] 10.4 Verify no clang-format warnings
