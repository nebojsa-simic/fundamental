		      ...
		      int *out_len)
{
	...
	*out_len = 0;
	...
	while (*out_len < (int)size - 1) {
		AsyncResult rr =
			fun_network_tcp_receive_exact(conn, &byte_buf, 1);
...
static void handle_client(TcpNetworkConnection conn, BrokerState *bs)
{
	...
	while (1) {
		...
		if (fun_string_length(line) >= 4 &&
		    line[0] == 'P' && line[1] == 'U' &&
		    line[2] == 'B' && line[3] == ' ') {
			String topic_start = line + 4;
			StringPosition sp = fun_string_index_of(
				topic_start, " ", 0);
			if (sp >= 0) {
				char topic_name[TOPIC_NAME_MAX];
				fun_string_substring(topic_start, 0,
						     (size_t)sp, topic_name,
						     sizeof(topic_name));
				String message = topic_start + sp + 1;
				Topic *t = find_or_create_topic(bs,
								topic_name);
				if (t) {
					fun_string_copy(message,
							t->last_message,
							sizeof(
								t->last_message));
					t->has_message = 1;
		...
		} else if (fun_string_length(line) >= 4 &&
			   line[0] == 'S' && line[1] == 'U' &&
			   line[2] == 'B' && line[3] == ' ') {
...
int main(void)
{
	...
	voidResult cr = fun_network_tcp_server_config(
		ar.value, (Memory)&bs, &config);
	...
	AsyncResult server =
		fun_network_tcp_listen(config, on_client_connected);
